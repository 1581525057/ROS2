...
/*
难题来了：在上台阶的时候，即从低到高的时候，我在2号方块有个点位：0 - 2上台阶的时候，用雷达读取这个点就能在对应的位置上去，但我的这个点位在2号的时候设为：x, y, 然后每个方块都是1.2米的正方形，我可以偏移点位，比如说2 - 5的点位为：x + 1.2, y 8的是x + 2.4, y, 但这只能前进的上20cm的点位，不能2 - 3这种平移的点位上20cm, 但我知道全部的此次的路径点：2 - 1 - 4 - 5 - 8 - 11 - 10等，而且我觉得可以把只能前进的点位旋转90度，得到平移的点位，你基于2号的x, y点位和根据代码中需要从低到高的路径和已知的路径，能不能在代码事先罗列出从低到高点位，我直接带入即可
3 2 1
6 5 4
9 8 7
12 11 10
